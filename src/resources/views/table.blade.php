        ...
        $(document).ready(function() {
            ...
            {{ (isset($id) ? $id : 'dtable') }} = $("#{{ (isset($id) ? $id : 'dtable') }}").DataTable({
                ...
                "buttons": [
                    ...
                    {
                        className: 'btn btn-warning btn-sm reload-table mb-2',
                ...
                "sDom": "<'row'<'col-12 text-right mb-4'B><'col-12 col-sm-6'l><'col-12 col-sm-6 d-none d-sm-block'p><'col-12't r><'col-12 dt-toolbar-footer mt-4'<'row'<'col-12 col-sm-6 mb-3 mb-sm-0'i><'col-12 col-sm-6 mb-3 mb-sm-0'p>>>>",
...
<table id="{{ (isset($id) ? $id : 'dtable') }}" class="dt-responsive nowrap {{ (isset($class) ? $class : 'table table-striped table-bordered') }} dataTable no-footer dtr-inline" style="width:100%">
    <thead>
                            ...
                            @else
                                @switch($column['filter']['type'])
                                    @case('text')
                                        ...
                                        <input type="text" class="form-control" placeholder="" value="{{ session()->get('datatable_'.url()->current())['columns'][$loop->index]['search']['value'] ?? '' }}"/>
                                    @break

                                    @case('date')
                                        <div class="input-group">
                                            <input type="text" class="form-control date_range" id="reportrange" placeholder="" value="{{ session()->get('datatable_'.url()->current())['columns'][$loop->index]['search']['value'] ?? '' }}" />
        ...
        @empty(!$columns)
            <tr>
                @foreach($columns as $_index => $column)
                    <th{{ !isset($column['width']) ? "" : " width=".$column['width']."%" }}{{ !isset($column['class']) ? "" : " class=".$column['class'] }}>{{ (isset($column['title']) ? $column['title'] : $_index) }}</th>
                @endforeach
            </tr>
        @endempty
    </thead>
    ...
    @if($deferLoading == true)
        @foreach($data['data'] as $d)
            <tr>
                @empty(!$columns)
                    @foreach($columns as $_index => $column)
                        <td{{ !isset($column['width']) ? "" : " width=".$column['width']."%" }}{{ !isset($column['class']) ? "" : " class=".$column['class'] }}>{!!  isset($d[$_index]) ? $d[$_index] : ''  !!}</td>
                    @endforeach
                @endempty
            </tr>
        @endforeach
